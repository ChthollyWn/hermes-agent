...
| `text_batch_delay_seconds` | `3.0` | Quiet period (seconds) before a buffered burst is flushed as one combined request. Applies to rapid text **and** photo/video/document (so an image followed by a caption like「这是谁啊」merges into one agent turn). iLink delivers messages individually; set `0` to dispatch each message immediately. |
